...
# ---- palette ----
PAPER="#EFECE4"; TRAV="#DCCDB3"; SAND="#C7B08C"; INK="#222216"; OLIVE="#4C5039"
GOLD="#C1A152"; MUTE="#8C8478"; RULE="#2C2620"; ENAMEL="#26241C"; WATER="#E9E7E0"

# ---- brand 'A' icon as base64 (for the QR centre) ----
with open(f"{ROOT}/icon-512.png","rb") as f:
    A_B64 = "data:image/png;base64," + base64.b64encode(f.read()).decode()
...
def verify_scan():
    ...
    for frac in (0.20, 0.18, 0.16, 0.14):
        ...
        a = Image.open(f"{ROOT}/icon-512.png").convert("RGB").resize((lside,lside))
...
def qr_svg_modules(x, y, size, dark=INK, light="#FFFFFF", logo=True, quiet=4):
    ...
    if logo:
        ...
        cx = x + total/2
        s.append(f'<rect x="{cx-lside/2-gap:.2f}" y="{cx-lside/2-gap:.2f}" width="{lside+2*gap:.2f}" height="{lside+2*gap:.2f}" rx="{gap:.2f}" fill="{light}"/>')
        s.append(f'<image x="{cx-lside/2:.2f}" y="{cx-lside/2:.2f}" width="{lside:.2f}" height="{lside:.2f}" href="{A_B64}"/>')
    return "".join(s), total
...
def titleblock(x,y,w, sheet, title, legend, h=152):
    ...
    s+=txt(rx,y+118,"DATE 2026-06",10,INK,weight="600")
    s+=txt(rx,y+131,"REV A · not for structural use",9.5,MUTE)
    return s
...
def build_sheet1():
    ...
    b.append(txt(54,90,"Honed travertine / marble plate · carved wordmark · gilded dot · inset QR",12.5,MUTE,ls="0.3"))
    b.append(line(54,104,W-54,104,RULE,0.8))

    # ---- 1 · FRONT ELEVATION (1:1) ----
    px,py,pw,ph=120,200,560,400
    ...
    b.append(rect(tx,ty,tile,tile,"#F2EFE8",ENAMEL,1.2))
    qs,_=qr_svg_modules(tx,ty,tile*N/(N+8),dark=INK,light="#F2EFE8",logo=True); b.append(qs)
    # dims
    b.append(dim_h(px,px+pw,py+ph+46,"560",ext_to=py+ph))
    ...
    b.append(dim_v(ay,ay+ah,ax+aw+14,"30",ext_to=ax+aw,side="right"))
    b.append(txt(ax,ay+ah+20,"(a) V-cut 60° · 6 deep · bare; gilded dot = cup + leaf",10.6,INK,ls="0.05"))
    bx,byy,bw,bh=sx+292,sy,250,112
    ...
    qsz=148; b.append(rect(sx,qy+12,qsz,qsz,"#F2EFE8",ENAMEL,1))
    qq,_=qr_svg_modules(sx,qy+12,qsz*N/(N+8),dark=INK,light="#F2EFE8",logo=True); b.append(qq)
    ...
    b.append(keylegend(sx, qy+qsz+40, [
        ...
        (5,"Honed travertine","matte · sealed · shelter from direct spray"),
    ...
    b.append(note_block(120,688,"GENERAL NOTES",[
        ...
        "Fixing: concealed 316 stainless pins + epoxy into masonry; ventilated cavity.",
    ...
    b.append(titleblock(740,H-180,540,"Sheet 1 of 2  ·  ENTRANCE THRESHOLD MARKER",
        ...
        [(TRAV,"Honed travertine / marble — plate"),(GOLD,"23.5ct gold leaf — dot only"),
...
def build_sheet2():
    ...
    b.append(txt(54,90,"Coastal sandstone standing stone · deep-cut wordmark · bare (no infill) · no QR",12.5,MUTE,ls="0.3"))
    b.append(line(54,104,W-54,104,RULE,0.8))
    ...
    b.append(rect(panx,pany,panW,panH,"#CFB892",RULE,1))
    gw_=panW*0.86; gx0=gx-gw_/2; gy0=pany+panH*0.30
    ...
    b.append(rect(mx-18,grdY,mw+36,emb,"#BBA788",RULE,1.1))      # footing
    ...
    b.append(note_block(W-540,430,"GENERAL NOTES",[
        ...
        "No QR here — rough texture + exposure defeat scanning; the marker is presence, not function.",
